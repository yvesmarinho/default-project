...
class CopilotAgentMerger:
    ...
    def can_merge(self, file_path: Path) -> bool:
        ...
        return (
            ...
            # Deve ter pelo menos .github/agents/file.md
    ...
    def merge(
        ...
        try:
            ...
            decision = self._should_merge(
                existing_fm, template_fm, existing_path.name)
    ...
    def _should_merge(
        ...
        if template_fm.tools:
            ...
            new_tools = [
                t for t in template_fm.tools if t not in existing_tools]
            if new_tools:
        ...
        if template_fm.description and existing_fm.description:
            if (
                ...
                len(template_fm.description) > len(
                    existing_fm.description) * 1.2
            ):
    ...
    def _merge_frontmatter(
        ...
        if template.handoffs:
            ...
            existing_agents = {h.get("agent")
                               for h in existing_handoffs if h.get("agent")}
    ...
    def _reconstruct_agent_file(
        ...
        for heading, section_content in content.sections.items():
            ...
            level = "###" if heading in [
                "Tool Preferences", "Examples"] else "##"
